...
import (
	...
	"github.com/kittenbark/smoldb/ysmol/internal/errors"
...
func (e *Encoder) isNeedQuoted(v string, numeric ...bool) bool {
	if e.isJSONStyle {
		return true
	}
	...
	if token.IsNeedQuoted(v, numeric...) {
		return true
	}
	return false
}

func (e *Encoder) EncodeString(v string, column int, numeric ...bool) *ast.StringNode {
	if e.isNeedQuoted(v, numeric...) {
		if e.singleQuote {
			v = quoteWith(v, '\'')
		} else {
			v = strconv.Quote(v)
...
func (e *Encoder) encodeMap(ctx context.Context, value reflect.Value, column int) (ast.Node, error) {
	...
	for i, k := range value.MapKeys() {
		keys[i] = k.Interface()
		if kind := k.Kind(); reflect.Int <= kind && kind <= reflect.Float64 {
			numeric = true
		}
	}
	...
	for _, key := range keys {
		...
		node.Values = append(node.Values, ast.MappingValue(
			nil,
			e.EncodeString(keyText, column, numeric),
			encoded,
		))
